...
for r=1:64
    for c=1:64
        B1((r-1)*64+c, 1:8, 1:8)=dct2(squeeze(B((r-1)*64+c,:,:)),8,8);
    end
end
...
for r=1:64
    for c=1:64
        C1((r-1)*64+c, 1:8, 1:8)=round(squeeze(B1((r-1)*64+c,:,:))./Q);
    end
end
...
for r=1:64
    for c=1:64
        C((r-1)*64+c, 1:8, 1:8)=squeeze(DC1((r-1)*64+c,:,:)).*Q;
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for r=1:64
    for c=1:64
        DB((r-1)*64+c, 1:8, 1:8)=idct2(squeeze(C((r-1)*64+c,:,:)),8,8);
    end
end
